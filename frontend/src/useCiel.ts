...
import { eventSocketURL, getChat, getDashboard, getState, sendMessage } from './api'
import type {
  ChatMessage,
  CielEvent,
  CielState,
...
function reduceEvent(state: CielState, event: CielEvent): CielState {
  ...
  switch (event.type) {
    case 'interaction.started':
      return { ...next, status: 'active', stage: 'router', iteration: 1, error: null, tools: [], routerDecision: null }
    case 'router.started':
